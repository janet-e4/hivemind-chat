<script lang="ts">
	...
	const roleLabel = (role: string) =>
		role === 'user' ? $i18n.t('User') : $i18n.t('Assistant');
	...
	const getModelActions = (message: any) => {
		const model = ($models ?? []).find((model: any) => model.id === message?.model);
	...
	$: branchMessages = history?.messages && history?.currentId ? createMessagesList(history, history.currentId) : [];
	$: visible = ($settings?.showConversationMinimap ?? true) && !$mobile && branchMessages.length > 1;
	$: previewMessage =
...
{#if visible}
	<nav
		class="conversation-minimap pointer-events-auto fixed right-4 top-24 z-20 hidden max-h-[calc(100vh-12rem)] items-center gap-2 md:flex"
		aria-label={$i18n.t('Conversation quick browse')}
	>
		{#if previewMessage}
			{@const branch = getBranchMeta(previewMessage)}
			<aside
				class="w-72 rounded-lg border border-gray-200 bg-white/95 p-3 text-xs shadow-xl backdrop-blur dark:border-gray-800 dark:bg-gray-950/95"
				on:mouseenter={() => {
				...
				<div class="mb-2 flex items-center justify-between gap-2">
					<div class="min-w-0 font-medium text-gray-800 dark:text-gray-100">
						...
						{#if previewMessage.model}
							<span class="font-normal text-gray-500 dark:text-gray-400">- {previewMessage.model}</span>
						{/if}
					</div>

					{#if branch.count > 1}
						<div class="shrink-0 text-gray-500 dark:text-gray-400">{branch.index}/{branch.count}</div>
					{/if}
				</div>
		...
		<div
			class="relative flex max-h-[calc(100vh-12rem)] w-11 flex-col gap-1 overflow-hidden rounded-lg border border-gray-200 bg-white/85 p-1.5 shadow-lg backdrop-blur dark:border-gray-800 dark:bg-gray-950/85"
		>
			<div
			...
			{#each branchMessages as message, idx (message.id)}
				...
				>
					...
					{#if branch.count > 1}
						<span class="absolute right-0 top-0 size-1.5 rounded-full bg-white dark:bg-gray-950"></span>
					{/if}
					{#if message.files?.length || message.sources?.length || message.citations?.length}
						<span class="absolute bottom-0 left-0 size-1.5 rounded-full bg-white/90 dark:bg-gray-950/90"></span>
					{/if}
				</button>
			{/each}
		</div>
